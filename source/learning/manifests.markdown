...
If you're interested in learning about ways to apply Puppet code across infrastructure on a large scale, you can also [download Puppet Enterprise and try it out on 10 nodes for free][pe_download]. PE offers a Web console or command line tools that make it easier to initiate Puppet runs on  multiple nodes.
