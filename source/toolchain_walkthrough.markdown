...
The Puppet docs site is written in Markdown. In order to get advanced HTML features, such as tables, we use the [kramdown][] Markdown interpreter. Kramdown understands vanilla Markdown along with a large set of additional markup conventions. kramdown is provided via a Ruby gem, which is installed during toolchain setup via the rubygems [bundler](#bundler).
...
Since we keep generated references for every patch release of Puppet, there will be times when you need to make a change to legacy generated references. 
...
- ActiveRecord: A few parts of Puppet have declared dependencies on ActiveRecord but do not actually use it and will not (Puppet 3 or later) raise an error if ActiveRecord isn't present. If generated reference pages come up blank, doublecheck for ActiveRecord. 
...
Look at the `:generate` task in the Rakefile


#### Subprojects (Mcollective, PuppetDB)

Some Puppet subprojects keep their own copies of their documentation. These projects are automatically checked out when the documentation is generated and no user intervention is required. 

##### What do I need to know about this?

**Day-to-day:** Usually nothing. These subprojects are handled as part of the standard `generate` Rake task. 

**Toolsmith:**

For the mcollective and PuppetDB docs generation, check the `externalsources` namespace in the Rakefile. 

#### Generated References/YARD Docs

The generated references and YARD docs are produced from inline documentation in Puppet.  You need to regenerate these with each patch release of Puppet. 
